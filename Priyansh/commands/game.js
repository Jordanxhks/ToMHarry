...
module.exports.run = async function ({ event, api, args }) {
  ...
  if (args[0].toLowerCase() == "delete") {
    global.moduleData.tictactoe.delete(threadID);
    return api.sendMessage("Board cleared!", threadID, messageID);
  }
  if (args[0].toLowerCase() == "continue") {
    if (!data.gameOn) return api.sendMessage("No data! used " + concak + " x/o to play new", threadID, messageID);
    return api.sendMessage({ body: "Reply number of boxes to mark", attachment: await displayBoard(data)}, threadID, (error, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        author: senderID,
        messageID: info.messageID
  ...
  if (!data.gameOn) {
    ...
    if (abc == "o") {
      ...
      api.sendMessage({ body: "You go first!\nReply number of boxes to mark", attachment: await displayBoard(newData)}, threadID, (error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
    ...
    if (abc == "x") {
      ...
      AIStart(newData);
      api.sendMessage({ body: "AI goes first!\nReply number of boxes to mark", attachment: await displayBoard(data)}, threadID,(error, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          author: senderID,
          messageID: info.messageID
  ...
  } else return api.sendMessage("This group already has a board.\nUse:\n" + concak + " continue -> continue\n" + concak + " delete -> ", threadID, messageID);
    }